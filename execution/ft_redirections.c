			...
			t_global *g, bool is_heredoc)
{
	if (cmd)
	{
		...
		cmd->final_io->infile = ft_strdup(node->infile);
		/*if (cmd->final_io->infile && cmd->final_io->infile[0] == '\0')
		{
			errmsg_cmd(cmd->final_io->infile, NULL, strerror(errno), false);
			ft_exit(g, NULL, EXIT_FAILURE);
		}*/
		if (is_heredoc)
		{
			cmd->final_io->heredoc_delimiter = ft_strdup(node->heredoc_delimiter);
			cmd->final_io->heredoc_quotes = node->heredoc_quotes;
		}
		if (cmd->final_io->heredoc_delimiter == NULL && access(node->infile, F_OK) == -1)
		{
