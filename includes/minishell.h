...
void						ft_split_env_variable(char *name_value,
								char **var_name, char **var_value);
char						*resolve_command_path(t_global *g, char *path,
								char *cmd);
void						ft_execute_child_proc(t_command *cmd,
								t_global *global, pid_t prev_pid);
bool						is_parent_builtin(t_command *command);
void						ft_safe_fork(t_global *g, t_command *cmd);
void						ft_split_child_parent_run(t_global *g,
...
void						ft_copy_input_to_final_io(t_io_fds *node,
								t_command *cmd, t_global *g, bool is_heredoc);
void						ft_copy_output_to_final_io(t_io_fds *node,
								t_command *cmd, t_global *g);
...
void						ft_handle_child_io_lst_node(t_global *g,
								t_command *cmd, t_io_fds *node);
void						ft_handle_parent_io_lst_node(t_global *g,
								t_command *cmd, t_io_fds *node);
void						ft_execute_cmd(t_global *g, t_command *cmd,
								pid_t prev_pid);
...
void						parse_word(t_command **curr_cmd,
								t_token **curr_token);
...
void						parse_pipe(t_global *g, t_command **curr_cmd,
								t_token **curr_token);
void						parse_heredoc(t_global *g, t_command **curr_cmd,
								t_token **curr_token);
void						ft_is_status_request(t_token *token,
								t_command *cmd);
...
int							create_args_default_mode(t_token **token_node,
								t_command *last_cmd);
t_token						*lst_new_token(char *str, char *str_backup,
								int type, int status);
void						lst_add_back_token(t_token **alst,
								t_token *new_node);
...
int							replace_var(t_token **token_node, char *var_value,
								int index);
char						*recover_val(t_token *token, char *str,
								t_global *global);
char						*replace_str_heredoc(char *str, char *var_value,
								int index);
void						copy_var_value(char *new_str, char *var_value,
								int *j);
char						*get_new_token_string(char *oldstr, char *var_value,
								int newstr_size, int index);
void						add_io_infile_data(t_global *global, t_command *cmd,
								char *f_name);
void						add_io_heredoc_data(t_global *g, t_command *cmd,
								char *delimiter);
...
void						ft_run_parent_builtins(t_command *cmd,
								t_global *global);
...
bool						ft_update_env_var_value(t_list *node,
								char *new_value);
bool						ft_add_env_var(t_global *g, char *name,
								char *value);
t_list						*ft_return_env_list_node_ptr(t_list *env,
								char *name);
bool						is_valid_var_name(char *var_name);
void						ft_add_new_env_var(char *var_name,
								t_global *global);
void						ft_mini_exit_wrapper(t_command *cmd, t_global *g);
void						ft_mini_export_wrapper(t_command *cmd,
								t_global *global);
