...
<<<<<<< Updated upstream:lexer/utils_for_tokens.c
=======
...
>>>>>>> Stashed changes:utils_for_tokens.c
...
int	add_word(t_token **token, char *str, int index, int start)
{
	...
	while (start < index)
		word[i++] = str[start++];
	word[i] = '\0';
	add_token(token, new_token(word, WORD, DEFAULT));
	return (0);
}

int	add_word_or_sep(int *i, char *str, int start, t_data *data)
{
	int	type;
...
int	tokenization(t_data *data, char *str)
{
	...
	while (++i < ft_strlen(str))
	{
		...
		if (status == DEFAULT)
			start = add__word_or_sep(&i, str, start, data);
	}
	if (status != DEFAULT)
	{
		if (status == DQUOTE)
